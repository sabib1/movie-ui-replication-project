import { NextRequest, NextResponse } from 'next/server';
import { db } from '@/db';
import { user } from '@/db/schema';
import { eq } from 'drizzle-orm';
import { getCurrentUser } from '@/lib/auth';

export async function POST(request: NextRequest) {
  try {
    // Authentication check
    const currentUser = await getCurrentUser(request);
    if (!currentUser) {
      return NextResponse.json(
        { error: 'Authentication required' },
        { status: 401 }
      );
    }

    // Parse request body
    const body = await request.json();
    const { movieId } = body;

    // Security check: reject if userId provided in body
    if ('userId' in body || 'user_id' in body) {
      return NextResponse.json(
        {
          error: 'User ID cannot be provided in request body',
          code: 'USER_ID_NOT_ALLOWED',
        },
        { status: 400 }
      );
    }

    // Validate required field
    if (!movieId) {
      return NextResponse.json(
        {
          error: 'movieId is required',
          code: 'MISSING_REQUIRED_FIELD',
        },
        { status: 400 }
      );
    }

    // Validate movieId is a number
    if (typeof movieId !== 'number' || isNaN(movieId)) {
      return NextResponse.json(
        {
          error: 'movieId must be a valid number',
          code: 'INVALID_MOVIE_ID',
        },
        { status: 400 }
      );
    }

    // Query user from database using authenticated user's ID
    const userRecord = await db
      .select()
      .from(user)
      .where(eq(user.id, currentUser.id))
      .limit(1);

    // Check if user exists
    if (userRecord.length === 0) {
      return NextResponse.json(
        { error: 'User not found', code: 'USER_NOT_FOUND' },
        { status: 404 }
      );
    }

    const foundUser = userRecord[0];

    // Parse existing bookmarks (handle null/empty as empty array)
    let bookmarks: number[] = [];
    if (foundUser.bookmarks) {
      try {
        const parsed = JSON.parse(foundUser.bookmarks);
        bookmarks = Array.isArray(parsed) ? parsed : [];
      } catch (parseError) {
        // If parsing fails, start with empty array
        bookmarks = [];
      }
    }

    // Check if movieId already exists in bookmarks
    if (bookmarks.includes(movieId)) {
      // Already bookmarked, return current bookmarks without adding duplicate
      return NextResponse.json({ bookmarks }, { status: 200 });
    }

    // Add movieId to bookmarks array
    bookmarks.push(movieId);

    // Stringify updated array back to JSON
    const updatedBookmarksJson = JSON.stringify(bookmarks);

    // Update user record with new bookmarks and updatedAt timestamp
    const updated = await db
      .update(user)
      .set({
        bookmarks: updatedBookmarksJson,
        updatedAt: new Date(),
      })
      .where(eq(user.id, currentUser.id))
      .returning();

    if (updated.length === 0) {
      return NextResponse.json(
        { error: 'Failed to update user bookmarks', code: 'UPDATE_FAILED' },
        { status: 500 }
      );
    }

    // Return updated bookmarks array
    return NextResponse.json({ bookmarks }, { status: 200 });
  } catch (error) {
    console.error('POST error:', error);
    return NextResponse.json(
      {
        error: 'Internal server error: ' + (error instanceof Error ? error.message : 'Unknown error'),
      },
      { status: 500 }
    );
  }
}